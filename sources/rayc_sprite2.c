...
void        last_render_sprite(t_rayc *rayc, int x, int i, t_data *data, t_vars *vars)
{
	int	y;
	int	d;
	...
	{
		d = (y * 256 - ft_atoi(data->y) * 128 + rayc->render.height * 128);
		...
		{
			if (x >= 0 && x < ft_atoi(data->x) && y >= 0 && y < ft_atoi(data->y))
			{
