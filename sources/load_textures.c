...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/02/18 08:42:14 by dsantama          #+#    #+#             */
/*   Updated: 2021/02/18 08:50:02 by dsantama         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(vars->rc.sprite[i].img_ptr = mlx_xpm_file_to_image(vars->mlx, \
	root_sprite, &vars->rc.sprite[i].width, &vars->rc.sprite[i].height)))
	{
		write(1, "Error\nSomething wrong with the sprite\n", 39);
		exit(0);
	}
	if (!(vars->rc.sprite[i].get_data = \
	(int *)mlx_get_data_addr(vars->rc.sprite[i].img_ptr, \
	&vars->rc.sprite[i].bits_per_pixel, \
...
void				set_texture(int x, t_vars *vars)
{
	...
	{
		tex_y = (y - vars->screen_height / 2 + vars->rc.lineheight / 2) * \
		tex_wall.height / vars->rc.lineheight;
		if (tex_y < 0)
			return ;
